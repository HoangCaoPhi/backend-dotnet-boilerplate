...
- A rule earns its place only if breaking it is **silent**: the compiler doesn't catch it and
  reading the code doesn't reveal it. If a wrong package won't compile, or one existing file shows
  the pattern, there is no rule to write.
